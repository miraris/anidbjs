...
class Mapper {
  ...
  static mapEpisode(episode) {
    return {
      ...
      titles: episode.title ? utils.ensureArray(episode.title).map(Mapper.mapTitle) : [],
      summary: episode.summary ? episode.summary : null,
    };
  }
  ...
  static mapCharacters(character) {
    return {
      ...
      seiyuu: character.seiyuu ? utils.ensureArray(character.seiyuu).map(Mapper.mapSeiyuu) : [],
    };
  }
  ...
  static mapResources(resource) {
    return {
      ...
      externalEntity: resource.externalentity
        ? utils.ensureArray(resource.externalentity).map(Mapper.mapExternalEntities)
        : [],
    };
  }
  ...
  static mapAnime(anime) {
    return {
      ...
      resources: utils.mapArray(anime.resources, 'resource', Mapper.mapResources),
      tags: utils.mapArray(anime.tags, 'tag', Mapper.mapTag),
      characters: utils.mapArray(anime.characters, 'character', Mapper.mapCharacters),
